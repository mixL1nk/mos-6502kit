...
impl Lexer {
    ...
    fn next_token(
        ...
        match c {
            ...
            ']' => {
                chars.next();
                Ok(TokenizeResult::Token(
                    self.create_token(Token::RightBracket),
                ))
            }
            '+' => {
                chars.next();
    ...
    fn process_macro_directive(
        &mut self,
        chars: &mut Peekable<Chars>,
        directive_token: TokenInfo,
    ) -> Result<TokenizeResult> {
        ...
        Ok(TokenizeResult::Tokens(vec![
            directive_token,
            macro_name_token,
        ]))
    }

    fn process_left_paren(
        &mut self,
        chars: &mut Peekable<Chars>,
        tokens: &[TokenInfo],
    ) -> Result<TokenizeResult> {
